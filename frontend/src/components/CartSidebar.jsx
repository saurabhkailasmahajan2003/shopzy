...
const CartSidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div 
        className={`fixed top-0 right-0 h-full w-full sm:w-[420px] bg-[#fefcfb] z-50 shadow-2xl flex flex-col border-l-2 border-[#120e0f] overflow-hidden transition-transform duration-300 ease-out ${
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {productsForFreebie > 0 && productsForFreebie <= freebieThreshold && (
            <div className="bg-green-100 border-b-2 border-[#120e0f] p-3">
              <p className="text-sm font-medium text-[#120e0f] text-center">
              ...
              <div className="mt-2 flex items-center gap-1">
                ...
                {[1, 2, 3].map((num) => (
                  <div 
                    key={num}
                    className={`w-5 h-5 flex items-center justify-center border-2 border-[#120e0f] ${
                      num <= cart.length ? 'bg-[#120e0f]' : 'bg-[#fefcfb]'
                    }`}
                  >
          ...
          <div className="p-4 space-y-4">
            {cart.length === 0 ? (
              <div className="text-center py-12">
                <svg className="w-16 h-16 mx-auto text-[#120e0f]/30 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                </svg>
                <p className="text-[#120e0f]/70 mb-4">Your cart is empty</p>
                <button
                  onClick={onClose}
                  className="px-4 py-2 border-2 border-[#120e0f] text-[#120e0f] font-medium hover:bg-[#120e0f] hover:text-[#fefcfb] transition-colors"
                >
                  Continue Shopping
                </button>
              </div>
              ...
              cart.map((item) => {
                ...
                return (
                  <div key={itemId} className="border-2 border-[#120e0f] bg-[#fefcfb] p-3">
                    <div className="flex gap-3">
                      {/* Product Image */}
                      <div className="w-20 h-20 flex-shrink-0 border-2 border-[#120e0f] overflow-hidden bg-[#fefcfb]">
                        <img
                          src={productImage}
                          alt={productName}
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm font-bold text-[#120e0f] truncate mb-1">
                          {productName}
                        </h3>
                        ...
                        <div className="flex items-center gap-2 mb-2">
                          {originalPrice > productPrice ? (
                            <>
                              ...
                              <span className="text-sm font-bold text-[#120e0f]">
                        ...
                        <div className="space-y-2">
                          ...
                          {(item.size || item.color || product.size || product.color) && (
                            <div className="text-xs text-[#120e0f]/70">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium text-[#120e0f] min-w-[24px] text-center">
                              {item.quantity}
                            </span>
                            <button
                              onClick={() => updateQuantity(itemId, item.quantity + 1)}
                      ...
                      <button
                        ...
                        className="self-start p-1 text-[#120e0f] hover:opacity-70 transition-opacity"
                      >
          ...
          {cart.length > 0 && (
            <div className="bg-green-100 border-t-2 border-b-2 border-[#120e0f] p-3 mx-4 mb-4">
              <p className="text-sm font-medium text-[#120e0f] text-center">
        ...
        {/* Footer - Fixed */}
        {cart.length > 0 && (
          <div className="border-t-2 border-[#120e0f] bg-[#fefcfb] p-4 space-y-4">
            {/* Grand Total */}
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold text-[#120e0f]">GRAND TOTAL</span>
              <div className="text-right">
                {cartTotal < freeShippingThreshold ? (
                  <>
                    <span className="text-sm text-[#120e0f]/50 line-through mr-2">
                ...
                <p className="text-xs text-[#120e0f]/70 mt-0.5">(Inc. All Taxes)</p>
              </div>
            </div>
            ...
            {/* Powered By */}
            <p className="text-xs text-[#120e0f]/60 text-center">
